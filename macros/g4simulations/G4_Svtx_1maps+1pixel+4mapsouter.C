	    ...
	    int verbosity = 0)
{
  ...
  float svtx_inner_radius = 2.3;   // radius of innermost ITS layer
  ...
  double svxrad[6] = {svtx_inner_radius, 3.91, 19.6, 24.5, 34.4, 64.0}; // 1st layer is layer one of ITS inner barrel, 2nd layer is Rachid's 19 ladder P0 layer
  double si_thickness[6] = {0.005, 0.0200, 0.005, 0.005, 0.005, 0.005};  // ALICE ITS upgrade Si thickness is 50 microns
  double length[6] = {27.0, 20.0, 88.0, 88.0, 148.0, 148.0}; // ALICE ladder lengths (stave - 2cm)
  ...
  double support_thickness[6] = {0.0036, 0.0154, 0.0108, 0.0108, 0.0108, 0.0108};
  ...
  radius += support_thickness[Max_si_layer] + no_overlapp;
    ...
    cout << "========================== G4_Svtx_1maps+1pixel+4mapsouter.C::Svtx() ==========================" << endl;
...
void Svtx_Cells(int verbosity = 0)
{
  ...
  double svxcellsizex[6] = {0.0020, 0.0050, 0.0020, 0.0020, 0.0020, 0.0020};
  double svxcellsizey[6] = {0.0020, 0.0425, 0.0020, 0.0020, 0.0020, 0.0020};
...
{
  // reconstructs the Svtx (6 layers)
  ...
  deadarea->set_hit_efficiency(1,0.77);  // Assumes 0.885 average live pixels, about 0.87 average pixel area
  ...
  hough->set_material(0, 0.003);  // phenix pixel layer thickness
