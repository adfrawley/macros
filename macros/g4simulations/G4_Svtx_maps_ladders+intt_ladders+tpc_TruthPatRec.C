  ...
  for (int ilayer = 0; ilayer < n_maps_layer; ilayer++)
    {
      ...
      lyr->set_double_param("pixel_x",0.0030);// pitch in cm, use 30 cm since charge sharing is simulated
      lyr->set_double_param("pixel_z",0.0030);// length in cm, use 30 cm since charge sharing is simulated
...
void Svtx_Cells(int verbosity = 0)
{
 ...
 PHG4MapsCellReco *maps_cells = new PHG4MapsCellReco("MAPS");
  ...
  // The maps cell sizes are set when the maps subsystem is called in Svtx()
...
void Svtx_Reco(int verbosity = 0, bool use_truth_pat_rec = false, bool do_refit = false)
{
  ...
  // The same clusterizer module is used for the maps and intt
  // The Svtx clusterizer runs different code for the cylinder cell, maps-ladder and intt-ladder cell objects that it finds on the node tree
